<?php
	...
	if (!file_exists('files/excel/Report_Elecs_Month.xls')) {
       exit('Phải tạo file Report_Elecs_Month.xls trước');
    }
    ...
    $excel->cellValign();
    $excel->changeCell("MỨC TIÊU ĐIỆN HÀNG NGÀY CỦA KHÁCH THUÊ TỪ (".$d1.'/'.$m1.'/'.$y1.") ĐẾN (".$d2.'/'.$m2.'/'.$y2.")",'A1');
	...
	//ngày 1 của tháng sau
		...
		foreach($cus as $c):
			...
			foreach($c['Room'] as $kr => $room):
				...
				$excel->changeCell($room['mactodien'],'D'.($row+$kr));
				$arrtieuthu= "=";
				$a = $this->requestAction('/elecs/getElecEx/'.date("Y-m-d",strtotime($y1."-".$m1."-".$d1)).'/'.date($y2."-".$m2."-".($d2+0)).'/'.$room["id"]);
				$arrkey = array_keys($a);
				$oldcel = 0;
				for($i=1;$i<=$d+1;$i++){
