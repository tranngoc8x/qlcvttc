...
<div id="plugin_acl">
	...
	if(!isset($no_acl_links))
	{
        ...
        $links = array();
        $links[] = $this->Html->link(__d('acl', 'Phân quyền', true), '/acl/aros/index', array('class' => ($selected == 'aros' )? 'selected' : null));
        $links[] = $this->Html->link(__d('acl', 'Thao tác', true), '/acl/acos/index', array('class' => ($selected == 'acos' )? 'selected' : null));
