...
<?php
	...
	echo $this->Html->link("Xuất ra file excel", array('controller'=>'elecs','action'=>'exportdate',($d1.'-'.$m1.'-'.$y1),($d2.'-'.$m2.'-'.$y2)))."<br/>";
	echo $this->Html->link("Xem biểu đồ thống kê chi tiết của tất cả các công ty", array('plugin'=>false,'admin'=>false,'controller'=>'elecs','action'=>'chart'))."<br/>";
	echo $this->Html->link("Xem biểu đồ thống kê chi tiết của từng công ty", array('plugin'=>false,'admin'=>false,'controller'=>'elecs','action'=>'chart_detail'));
	$mom1 = nhuan($y1);
	$mom2 = nhuan($y2);
?>
<table class="sort-table" cellspacing="0" >
		...
		<tr>
			...
			<?php
			$a = $this->requestAction('/elecs/getElecEx/'.date("Y-m-d",strtotime($y1."-".$m1."-".$d1)).'/'.date($y2."-".$m2."-".($d2+0)).'/'.$k["id"]);
