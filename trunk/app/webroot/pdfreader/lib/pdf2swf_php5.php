	...
	*/
	public function convert($doc,$page)
	{
		...
		if(	!validPdfParams($pdfFilePath,$doc,$page) ) {
			$s="Incorrect document file specified, file may not exist or insufficient permissions to read file";
			return $s;
		}

		// Document is not a PDF, convert with OpenOffice first
		if(substr_compare($doc,'pdf', -3, 3) === -1){
			if(!file_exists($this->configManager->getConfig('path.pdf') . $doc . '.pdf')){
				$oocommand = $this->configManager->getConfig('cmd.conversion.openoffice');
				$oocommand = str_replace("{path.pdf}",$this->configManager->getConfig('path.pdf'),$oocommand);
				$oocommand = str_replace("{pdffile}",$doc,$oocommand);
				
				//echo $oocommand;
				//die;
				exec($oocommand,$output,$return_var);
			}
			$doc = $doc . '.pdf';
		}
